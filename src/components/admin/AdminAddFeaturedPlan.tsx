...
export function AdminAddFeaturedPlan({ onCancel, onSuccess }: { onCancel: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Error creating featured plan:", err);
    ...
    <div className="bg-slate-50 min-h-full">
      <div className="bg-white rounded-lg border shadow-sm max-w-5xl mx-auto p-6 my-6">
        <div className="flex justify-between items-center mb-6 pb-4 border-b">
          <h2 className="text-xl font-bold tracking-tight text-slate-900 uppercase">Add Featured Plan</h2>
          <div className="text-sm text-slate-500">
            Home / Featured Plans / Add Featured Plan
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-white p-6 rounded-md border border-slate-100 shadow-sm space-y-6">
            <h3 className="font-semibold text-slate-800 mb-4 pb-2 border-b">Add Featured Plan</h3>
