        ...
        <div className="flex-1 w-full bg-background p-8 md:p-12 lg:p-16">
            <div className="max-w-5xl mx-auto space-y-8">
                ...
                <div className="flex justify-between items-end mb-2">
                    <h1 className="text-3xl md:text-4xl font-bold tracking-tight text-white">Company Dashboard</h1>
                    {isApproved && (
                        !isEditing ? (
                            <Button variant="outline" className="border-white/20 bg-white/5" onClick={() => {
                                setEditForm({
                ...
                {isApproved && (
                    <div className="bg-green-500/10 border border-green-500/30 p-6 rounded-xl flex items-start flex-col sm:flex-row gap-4 relative overflow-hidden">
                        <div className="absolute inset-0 bg-green-500/5 z-0" />
                        <CheckCircle2 className="w-8 h-8 text-green-500 shrink-0 mt-1 relative z-10" />
                        <div className="relative z-10">
                            <h3 className="text-xl font-bold text-green-500">Profile Verified & Active</h3>
                            <p className="text-white/80 mt-2 leading-relaxed max-w-2xl">
                ...
                {stripeSimulating && (
                    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center">
                        ...
                        <h2 className="text-3xl font-bold text-white mb-2 tracking-tight">Processing Payment...</h2>
                    ...
                    <Card className="bg-white/5 border-white/10 backdrop-blur-md shadow-xl">
                        ...
                        <CardContent className="space-y-6 pt-6">
                            <div>
                                ...
                                {isEditing ? <Input value={editForm.businessName} onChange={e => setEditForm({ ...editForm, businessName: e.target.value })} className="bg-white/5 border-white/20" /> : <p className="text-2xl text-white font-bold">{partnerData.businessName}</p>}
                            </div>
                            <div className="grid grid-cols-2 gap-6 bg-black/40 p-4 rounded-lg border border-white/5">
                                <div>
                                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Group selection</p>
                                    <p className="text-white font-medium capitalize">{partnerData.selectedGroup?.replace(/_/g, ' ') || "N/A"}</p>
                                </div>
                                <div>
                                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">Plan selected</p>
                                    <p className="text-white font-medium capitalize">{partnerData.selectedPlan?.replace(/_/g, ' ') || "N/A"}</p>
                                </div>
                            </div>
                            {partnerData.selectedAddon && partnerData.selectedAddon !== "none" && partnerData.selectedAddon !== "" && (
                                <div className="bg-primary/10 border border-primary/20 p-4 rounded-lg">
                                    ...
                                    <Badge variant="outline" className="mt-1 border-primary/50 text-white bg-black/50 px-3 py-1 shadow-inner">{partnerData.selectedAddon.replace(/_/g, ' ')}</Badge>
                                </div>
                            )}
                            <div>
                                ...
                                {isEditing ? <Input value={editForm.businessAddress} onChange={e => setEditForm({ ...editForm, businessAddress: e.target.value })} className="bg-white/5 border-white/20" /> : <p className="text-white/90 font-medium pl-6">{partnerData.businessAddress || "N/A"}</p>}
                            </div>
                        </CardContent>
                    </Card>
                    ...
                    <Card className="bg-white/5 border-white/10 backdrop-blur-md shadow-xl">
                        ...
                        <CardContent className="space-y-6 pt-6">
                            <div>
                                ...
                                {isEditing ? <Input value={editForm.primaryName} onChange={e => setEditForm({ ...editForm, primaryName: e.target.value })} className="bg-white/5 border-white/20" /> : <p className="text-xl text-white font-bold">{partnerData.primaryName}</p>}
                            </div>
                            <div className="space-y-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-white font-medium">{partnerData.primaryEmail}</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        {isEditing ? <Input value={editForm.phoneNumber} onChange={e => setEditForm({ ...editForm, phoneNumber: e.target.value })} className="bg-white/5 border-white/20 mt-1 h-8" /> : <p className="text-white font-medium">{partnerData.phoneNumber || partnerData.businessPhone || "N/A"}</p>}
                                    </div>
                                </div>
                            </div>
                            ...
                            {(partnerData.secondaryName || partnerData.secondaryEmail) && (
                                <div className="pt-6 border-t border-white/10 mt-6">
                                    ...
                                    <div className="bg-black/40 p-4 rounded-lg border border-white/5">
                                        {partnerData.secondaryName && <p className="text-white font-semibold mb-1">{partnerData.secondaryName}</p>}
                                        {partnerData.secondaryEmail && <p className="text-white/80 text-sm">{partnerData.secondaryEmail}</p>}
                                    </div>
                                </div>
                            )}
                        </CardContent>
                        ...
                        {offerings.length === 0 ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                {offerings.map(offering => (
                                    <Card key={offering.id} className="bg-black/40 border-white/10">
                                        ...
                                        <CardContent className="pt-4 space-y-3 text-sm">
                                            <div className="flex flex-col gap-1 text-white/80">
                                                ...
                                                <div className="flex flex-wrap gap-2">
                                                    {offering.bioSafetyLevel?.map((b: string, i: number) => <Badge variant="secondary" key={i} className="bg-white/10">{b}</Badge>) || "N/A"}
                                                </div>
                                            </div>
                                            <div className="flex flex-col gap-1 text-white/80">
                                                <span className="text-muted-foreground uppercase text-[10px] tracking-wider font-bold">Certifications</span>
                                                <div className="flex flex-wrap gap-2">
                                                    {offering.certifications?.map((c: string, i: number) => <Badge variant="outline" key={i} className="border-white/20">{c}</Badge>) || "N/A"}
                                                </div>
                                            </div>
                                            <div className="flex flex-col gap-1 text-white/80">
                                                <span className="text-muted-foreground uppercase text-[10px] tracking-wider font-bold">Categories</span>
                                                <p className="font-medium text-white">{offering.categories?.join(', ') || "N/A"}</p>
                                            </div>
                                        </CardContent>
                                    </Card>
                                ))}
