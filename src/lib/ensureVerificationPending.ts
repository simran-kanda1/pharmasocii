...
export type EnsureVerificationResult =
  | { ok: true; skipped?: string; hasLink?: boolean; status?: string; message?: string | null }
...
export async function ensureVerificationPending(): Promise<EnsureVerificationResult> {
  const fn = httpsCallable<
    Record<string, never>,
    { ok: boolean; skipped?: string; hasLink?: boolean; status?: string; message?: string | null }
  >(functions, "ensureVerificationPending");
  try {
    const res = await fn({});
    const data = res.data;
    if (data.ok) return { ok: true, skipped: data.skipped, hasLink: data.hasLink, status: data.status, message: data.message };
