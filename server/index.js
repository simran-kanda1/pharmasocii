...
async function applyListingPatchEverywhere(partnerId, collectionName, listingId, patch) {
    ...
    const snaps = await Promise.all(uniqueRefs.map((r) => r.get()));
    let updated = 0;
    for (let i = 0; i < uniqueRefs.length; i++) {
        if (!snaps[i].exists) continue;
        await uniqueRefs[i].set(patch, { merge: true });
        updated += 1;
    }
    return { updated, refs: uniqueRefs.filter((_, i) => snaps[i].exists) };
}

async function resolveStandaloneSpotlightForCancel(partnerRef, listingId, collectionName, listingData) {
    let linkedFeatureId = String(listingData?.selectedAddon || listingData?.featuredPlacement || "").trim() || null;
    let featureSubId = toStripeSubscriptionId(listingData?.featureSpotlightStripeSubscriptionId);
    ...
    if (listingId) {
        const byListing = await featuresRef
            ...
            .limit(20)
            .get();
        featureDocs = byListing.docs.filter((d) => {
            const fd = d.data() || {};
            if (fd.source === "included_plan") return false;
            if (collectionName && fd.collectionName && fd.collectionName !== collectionName) return false;
            return fd.source === "spotlight_addon" || Boolean(fd.stripeSubscriptionId);
        });
    }
    ...
    const primaryFeatureDoc = featureDocs.find((d) => (d.data() || {}).source === "spotlight_addon") || featureDocs[0];
    if (primaryFeatureDoc) {
        const fd = primaryFeatureDoc.data() || {};
        if (!linkedFeatureId && fd.featureId) linkedFeatureId = fd.featureId;
        if (!featureSubId) featureSubId = toStripeSubscriptionId(fd.stripeSubscriptionId);
    }

    const hasPaidStandalone =
        Boolean(featureSubId) ||
        Boolean(listingData?.lastFeaturePaymentReceivedAt) ||
        Boolean(toDateValue(listingData?.featureSpotlightPaidThrough));

    return { linkedFeatureId, featureSubId, featureDocs, hasPaidStandalone };
}
...
function getInvoicePeriodStartDate(invoice) {
    const firstLine = invoice?.lines?.data?.[0];
    const periodStartSeconds = firstLine?.period?.start || null;
    if (!periodStartSeconds) return null;
    return new Date(periodStartSeconds * 1000);
}

/**
 * Resolve the billing period end for an invoice.
 * Prefer the invoice line item period (immutable, exact cycle) over
 * subscription.current_period_end (which reflects the live state and may
 * have already advanced by the time a delayed payment is processed).
 */
...
/**
 * Resolve the billing period start for an invoice.
 * Same preference order as resolveBillingPeriodEndFromStripe.
 */
async function resolveBillingPeriodStartFromStripe(stripeClient, invoice, subscriptionId) {
    const fromInvoice = getInvoicePeriodStartDate(invoice);
    if (fromInvoice) return fromInvoice;
    const subId = toStripeSubscriptionId(subscriptionId);
    if (subId && stripeClient) {
        try {
            ...
            console.error("resolveBillingPeriodStartFromStripe:", err?.message || err);
        }
    }
    return null;
}

/** Live subscription period — authoritative after renewals (invoice line period can lag one cycle). */
function resolveBillingPeriodFromSubscription(subscription) {
...
async function resolveBillingPeriodFromStripe(stripeClient, invoice, subscriptionId, subscription = null) {
    let billingPeriodStart = await resolveBillingPeriodStartFromStripe(stripeClient, invoice, subscriptionId);
    let billingPeriodEnd = await resolveBillingPeriodEndFromStripe(stripeClient, invoice, subscriptionId);
    ...
    const live = resolveBillingPeriodFromSubscription(sub);
    if (live.billingPeriodStart) billingPeriodStart = live.billingPeriodStart;
    if (live.billingPeriodEnd) billingPeriodEnd = live.billingPeriodEnd;
    return { billingPeriodStart, billingPeriodEnd, subscription: sub };
}

/** Prefer Stripe subscription period end on initial checkout (handles month-end anchors). */
async function resolvePlanBillingPeriodEndFromCheckout(session, planId, startDate = new Date()) {
    const rawSub = session?.subscription;
    if (rawSub && typeof rawSub === "object" && rawSub.current_period_end) {
        return new Date(rawSub.current_period_end * 1000);
    }
    const subId = toStripeSubscriptionId(rawSub);
    if (subId) {
        const fromStripe = await resolveBillingPeriodEndFromStripe(stripe, null, subId);
...
function isSpotlightCancelPending(listingData) {
    if (!listingData?.featureSpotlightCancelPending) return false;
    const end =
...
/** Firestore payload that clears standalone spotlight residue on a listing. */
function listingSpotlightClearPatch() {
...
/**
 * Clear leftover spotlight fields when the Stripe add-on is gone / paid window ended,
 * so a new plan on the same listing can purchase a feature again.
 * Keeps fields if a distinct spotlight subscription is still live.
 */
async function reconcileStaleSpotlightOnListing({
    partnerId,
    collectionName,
    listingId,
    listingData,
    keepIncludedSpotlight = null,
}) {
    if (!partnerId || !collectionName || !listingId || !listingData) {
        return { cleared: false, listingData };
    }
    ...
    const currentAddon = String(listingData.selectedAddon || listingData.featuredPlacement || "").trim();
    if (keepIncludedSpotlight && currentAddon === keepIncludedSpotlight && !featureSubId) {
        return { cleared: false, listingData, reason: "included_spotlight" };
    }
    ...
    const hasResidue = Boolean(
        ...
        featureSubId ||
        listingData.featureSpotlightSubscriptionItemId ||
        listingData.featureSpotlightPaidThrough ||
        listingData.lastFeaturePaymentReceivedAt,
    );
    ...
    if (windowOver || !featureSubLive) {
        await applyListingPatchEverywhere(partnerId, collectionName, listingId, listingSpotlightClearPatch());
        try {
            const partnerRef = db.collection("partnersCollection").doc(partnerId);
            ...
            for (const fDoc of featSnap.docs) {
                ...
                await fDoc.ref.set(
                    {
                        active: false,
                        cancelPending: admin.firestore.FieldValue.delete(),
            ...
            console.warn("   ⚠ Could not deactivate stale feature docs:", featErr?.message || featErr);
        }
        const cleared = { ...listingData };
        delete cleared.selectedAddon;
        delete cleared.featuredPlacement;
        delete cleared.featureSpotlightCancelPending;
        delete cleared.featureSpotlightAccessEnd;
        delete cleared.featureSpotlightPaidThrough;
        delete cleared.featureSpotlightBillingPeriodStart;
        delete cleared.lastFeaturePaymentReceivedAt;
        delete cleared.featureSpotlightStripeSubscriptionId;
        delete cleared.featureSpotlightSubscriptionItemId;
        cleared.isFeatured = false;
        console.log(`   ✓ Cleared stale spotlight fields on listing ${listingId}`);
        return { cleared: true, listingData: cleared };
    }
...
async function resolveSpotlightAddonAccessEndDate(listingData, plan) {
    ...
    if (featureSubId) {
        try {
            ...
            console.warn("resolveSpotlightAddonAccessEndDate:", err?.message || err);
        }
    }
    const paidThrough = toDateValue(listingData?.featureSpotlightPaidThrough);
    ...
    if (!end && plan?.stripeSubscriptionId) {
        try {
            ...
            console.warn("resolveSpotlightAddonAccessEndDate plan sub:", err?.message || err);
        }
    }
...
async function scheduleSpotlightEndWithPlanCancel(partnerRef, partnerId, plan, planAccessEnd) {
    ...
    if (!spotlightCtx.hasPaidStandalone || !spotlightCtx.linkedFeatureId) {
        ...
        if (listingData.featureSpotlightCancelPending || listingData.selectedAddon || listingData.featuredPlacement) {
    ...
    if (distinctFeatureSubId) {
        try {
            ...
            console.warn("   ⚠ Could not set Stripe spotlight cancel on plan cancel:", stripeErr?.message || stripeErr);
            try {
                await stripe.subscriptions.update(distinctFeatureSubId, { cancel_at_period_end: true });
            } catch (_) {
                /* best effort */
    ...
    switch (event.type) {
        case "checkout.session.completed": {
            ...
            if (session.metadata?.upgradeFlow === "true") {
                ...
                if (upgradeListingId && upgradeCollectionName) {
                    ...
                    if (listingRef) {
                        ...
                        const accessThrough = upgradedSubscription.current_period_end
                        ...
                        const spotlightPatch = await applyListingSpotlightAfterPlanUpgrade({
                            listingRef,
                            listingData,
                            partnerRef,
                            listingId: upgradeListingId,
                ...
                if (!planSnap.empty) {
                    await planSnap.docs[0].ref.set({
                        ...
                        billingPeriodStart: upgradedSubscription.current_period_start
                            ? new Date(upgradedSubscription.current_period_start * 1000)
            ...
            if (featureId) {
                ...
                if (!listingRef) {
                    ...
                    if (!includedSpotlight && listingData) {
                        await reconcileStaleSpotlightOnListing({
                            partnerId,
                            collectionName: resolvedCollectionName,
                            listingId,
                            listingData: {
                                ...listingData,
                                stripeSubscriptionId: listingPaymentUpdate.stripeSubscriptionId,
                            },
                        });
                    }
                    if (includedSpotlight) {
        ...
        case "customer.subscription.deleted": {
            ...
            async function applyPlanSubscriptionDeletedToListing(listingRef, partnerIdForListing, listingId, collectionName) {
                const listingSnap = await listingRef.get();
                const listingData = listingSnap.exists ? listingSnap.data() || {} : {};
                if (partnerIdForListing) {
                    const endingPlans = await db.collection("partnersCollection")
                        .doc(partnerIdForListing)
                        .collection("planCollection")
                ...
                const featureSubId = toStripeSubscriptionId(listingData.featureSpotlightStripeSubscriptionId);
                const endedPlanSubId = toStripeSubscriptionId(subscription.id);
                // A plan ending always ends its spotlight. Do not leave a still-live add-on on the public site.
                if (featureSubId && featureSubId !== endedPlanSubId) {
                    try {
                        ...
                        console.warn(
                            "   ⚠ Could not cancel leftover spotlight subscription after plan end:",
                ...
                if (partnerIdForListing && listingId) {
                    const featSnap = await db.collection("partnersCollection")
                        .doc(partnerIdForListing)
                        .collection("featuresCollection")
                        .where("listingId", "==", listingId)
                    ...
                    for (const fDoc of featSnap.docs) {
                        const fd = fDoc.data() || {};
                        if (collectionName && fd.collectionName && fd.collectionName !== collectionName) continue;
                        if (fd.active === false) continue;
                        await fDoc.ref.set({
                            active: false,
                            cancelPending: false,
...
function spotlightTierFromId(featureId) {
    return FEATURE_SPOTLIGHT_TIER[String(featureId || "").trim()] || 0;
}

function spotlightIdFromTier(tier) {
...
function isEventOrJobPlanId(planId) {
...
/**
 * Mirror a Stripe spotlight subscription that is set to cancel onto the listing.
 * Never extends an end date that was already shortened (for example by a plan cancel).
 */
async function syncSpotlightCancelFromStripeSubscription(subscription) {
    if (!subscription?.id) return;
    const cancelling = Boolean(subscription.cancel_at_period_end || subscription.cancel_at);
    if (!cancelling) return;

    const stripeEnd = subscription.cancel_at
    ...
    const featureDocs = await findSpotlightAddonFeatureDocs(subscription.id);
    for (const fDoc of featureDocs) {
        const fd = fDoc.data() || {};
        const partnerId = partnerIdFromPartnersPlanRef(fDoc.ref);
        const existingEnd = toDateValue(fd.accessThrough);
        const nextEnd = existingEnd && existingEnd.getTime() < stripeEnd.getTime() ? existingEnd : stripeEnd;
        const cancelScope = fd.cancelScope === "plan" ? "plan" : "feature";
        await fDoc.ref.set(
            {
                cancelPending: true,
                accessThrough: nextEnd,
                cancelScope,
            },
            { merge: true },
        );
...
function shouldCancelStandaloneSpotlightAfterPlanUpgrade({
    ...
    if (!standaloneSubId) return false;
    const standaloneTier = spotlightTierFromId(currentDisplay);
    if (standaloneTier <= 0) return false;
    const includedTier = spotlightTierFromId(includedSpotlight);
    if (includedTier > 0 && standaloneTier <= includedTier) return true;
    if (currentDisplay === "both" && includedSpotlight === "home_page") return true;
    return false;
}

async function scheduleStandaloneSpotlightAddonCancellation({
    listingRef,
    listingData,
    partnerRef,
    listingId,
    ...
    const subId = toStripeSubscriptionId(listingData?.featureSpotlightStripeSubscriptionId);
    if (subId) {
        try {
            await stripe.subscriptions.update(subId, { cancel_at_period_end: true });
        } catch (err) {
            console.warn("scheduleStandaloneSpotlightAddonCancellation:", err?.message || err);
        }
    }
    ...
    const featuresRef = partnerRef.collection("featuresCollection");
    const byListing = await featuresRef.where("listingId", "==", listingId).where("active", "==", true).get();
    for (const fDoc of byListing.docs) {
        const d = fDoc.data() || {};
        if (d.source !== "spotlight_addon") continue;
        await fDoc.ref.set(
            {
                cancelPending: true,
...
async function applyListingSpotlightAfterPlanUpgrade({
    ...
    if (effectiveSpotlight) {
        patch.selectedAddon = effectiveSpotlight;
        patch.featuredPlacement = effectiveSpotlight;
        patch.isFeatured = true;
    }

    const shouldCancelStandalone = shouldCancelStandaloneSpotlightAfterPlanUpgrade({
        standaloneSubId,
        currentDisplay,
        includedSpotlight,
    });

    if (shouldCancelStandalone && accessThrough) {
        await scheduleStandaloneSpotlightAddonCancellation({
            listingRef,
            listingData,
            partnerRef,
            listingId,
            accessEnd: accessThrough,
        });
    }

    if (includedSpotlight) {
        if (!shouldCancelStandalone) {
    ...
    if (effectiveSpotlight && !standaloneSubId) {
        await deactivateSupersededPartnerFeatures(partnerRef, listingId, effectiveSpotlight);
    }
...
function resolveFeatureCheckoutAmount(
    targetFeatureId,
    listingAddonRaw,
    featuredPlacementRaw,
    listingData = null,
    options = {}
) {
    ...
    if (effectiveTier >= newTier && effectiveTier > 0) {
    ...
    if (effectiveFeatureId === targetFeatureId) {
        return { error: "You already have this spotlight tier." };
    }
    ...
    if (listingData) {
        const paidAt =
            ...
            toDateValue(listingData.lastPaymentReceivedAt);
        if (paidAt) {
            ...
            unitAmount = Math.max(Math.round(diff * ratio), 50);
        }
    }
    return { unitAmount, previousFeatureId: effectiveFeatureId, isUpgrade: true };
}
...
async function finalizeFeatureUpgradeAfterPayment({
    ...
    if (listingRef) {
        ...
        if (partnerId && collectionName && listingId) {
            await applyListingPatchEverywhere(partnerId, collectionName, listingId, listingPatch);
        } else {
...
async function upsertIncludedPlanFeature(partnerRef, {
    ...
    if (!existingDoc) {
        ...
        const existingEnd = toDateValue(existing.accessThrough);
        const nextEnd = toDateValue(accessThrough);
        const keepCancel = Boolean(existing.cancelPending);
        const updatePayload = {
            ...basePayload,
            deactivatedAt: admin.firestore.FieldValue.delete(),
        };
        if (keepCancel) {
            updatePayload.cancelPending = true;
            updatePayload.cancelScope = existing.cancelScope || "plan";
            if (existingEnd && (!nextEnd || existingEnd.getTime() <= nextEnd.getTime())) {
                updatePayload.accessThrough = existingEnd;
            } else if (nextEnd) {
                updatePayload.accessThrough = nextEnd;
            }
        } else {
            updatePayload.cancelPending = false;
            updatePayload.cancelScope = admin.firestore.FieldValue.delete();
            updatePayload.accessThrough = accessThrough || admin.firestore.FieldValue.delete();
        }
...
async function tryProcessSpotlightAddonInvoicePaid({
    ...
    let featureSub = null;
    try {
        featureSub = await stripe.subscriptions.retrieve(subscriptionId);
    } catch (subErr) {
        console.warn("tryProcessSpotlightAddonInvoicePaid retrieve:", subErr?.message || subErr);
    }
    const spotlightCancelling = Boolean(
        featureSub?.cancel_at_period_end ||
        featureSub?.cancel_at ||
        addonDocs.some((d) => (d.data() || {}).cancelPending) ||
        listingHint?.featureSpotlightCancelPending,
    );
    const spotlightCancelCap = featureSub?.cancel_at
        ? new Date(featureSub.cancel_at * 1000)
        : (featureSub?.cancel_at_period_end && featureSub?.current_period_end
            ? new Date(featureSub.current_period_end * 1000)
            : (toDateValue(listingHint?.featureSpotlightAccessEnd) || null));
    if (
        billingReason === "subscription_cycle" &&
        spotlightCancelling &&
        spotlightCancelCap &&
        periodEnd &&
        periodEnd.getTime() > spotlightCancelCap.getTime() + 60 * 1000
    ) {
        console.log(`   ℹ Skipping spotlight renewal for ${subscriptionId}; it would extend past the scheduled end.`);
        return;
    }
    ...
    if (addonDocs.length === 0) {
        ...
        for (const col of listingCollections) {
            ...
            for (const lDoc of snap.docs) {
                ...
                console.log(`   ✓ Spotlight renewal applied via listing fallback: ${col}/${lDoc.id}`);
            }
        }
        const partners = await db.collection("partnersCollection").get();
        for (const pDoc of partners.docs) {
            const embeddedCols = ["businessOfferingsCollection", "consultingServicesCollection", "consultingCollection", "eventsCollection", "jobsCollection"];
            for (const col of embeddedCols) {
                const snap = await pDoc.ref.collection(col)
                    .where("featureSpotlightStripeSubscriptionId", "==", subscriptionId)
                    .get();
                for (const lDoc of snap.docs) {
                    fallbackFound = true;
                    ...
                    await lDoc.ref.set(patch, { merge: true });
                    console.log(`   ✓ Spotlight renewal applied via embedded listing fallback: ${pDoc.id}/${col}/${lDoc.id}`);
                }
            }
        }
        if (!fallbackFound) {
            console.warn(`   ⚠ No spotlight add-on feature record or listing found for subscription ${subscriptionId}`);
        }
        return;
    }
            ...
            if (planAccessEnd) {
                try {
                    const cascade = await scheduleSpotlightEndWithPlanCancel(
                        db.collection("partnersCollection").doc(partnerId),
                        partnerId,
                        { ...planData, stripeSubscriptionId: subId },
                        planAccessEnd,
                    );
                    if (cascade?.cancelledFeature || cascade?.alreadyScheduled) {
                        await planDoc.ref.set({ spotlightEndsWithPlan: true }, { merge: true });
                    }
                } catch (cascadeErr) {
                    console.warn("   ⚠ Could not align spotlight end with plan cancel:", cascadeErr?.message || cascadeErr);
                }
            }
        }
    }
...
async function processSubscriptionInvoicePaid(invoice, options = {}) {
    ...
    let effectivePlanSnap = planSnap;

    if (planSnap.empty && customerId) {
        console.warn(`   ⚠ No plan doc matched stripeSubscriptionId ${subscriptionId}; trying stripeCustomerId fallback.`);
        const custId = toStripeCustomerId(customerId);
        if (custId) {
            const fallbackSnap = await db.collectionGroup("planCollection")
                .where("stripeCustomerId", "==", custId)
                .where("active", "==", true)
                .get();
            if (!fallbackSnap.empty) {
                effectivePlanSnap = fallbackSnap;
                console.warn(`   ✓ Found ${fallbackSnap.size} active plan(s) via stripeCustomerId ${custId}; fixing stripeSubscriptionId.`);
            }
        }
    }

    const planSubCancelling = Boolean(
        invoiceSubscription?.cancel_at_period_end || invoiceSubscription?.cancel_at,
    );
    const planCancelCap = invoiceSubscription?.cancel_at
        ? new Date(invoiceSubscription.cancel_at * 1000)
        : (invoiceSubscription?.cancel_at_period_end && invoiceSubscription?.current_period_end
    ...
    if (
        ...
        planCancelCap &&
        billingPeriodEnd &&
        billingPeriodEnd.getTime() > planCancelCap.getTime() + 60 * 1000
    ) {
        console.log(`   ℹ Skipping plan renewal extension for ${subscriptionId}; it would extend past the scheduled end.`);
        return;
    }

    if (effectivePlanSnap.empty) {
        await tryProcessSpotlightAddonInvoicePaid({
            invoice,
            subscriptionId,
            customerId,
    ...
    for (const planDoc of effectivePlanSnap.docs) {
        const planData = planDoc.data() || {};
        const partnerId = partnerIdFromPartnersPlanRef(planDoc.ref);
        if (!partnerId) continue;
        ...
        if (
            billingPeriodEnd &&
            (isRenewal || !existingEnd || billingPeriodEnd.getTime() >= existingEnd.getTime())
        ) {
        ...
        if (
            billingPeriodStart &&
            (isRenewal || !existingStart || billingPeriodStart.getTime() >= existingStart.getTime())
        ) {
...
async function finalizeJobListingPlanUpgradeWrites({
    ...
    Object.assign(listingPatch, spotlightPatch);
...
app.post("/api/create-feature-checkout", async (req, res) => {
    try {
        ...
        const listingDataRaw = listingSnap.data() || {};
        if (listingDataRaw.status === "pending_payment" || listingDataRaw.active === false) {
            return res.status(400).json({ error: "Complete listing plan payment before purchasing a feature add-on." });
        }

        // Drop leftover spotlight fields from a previous plan so repurchase works.
        const reconciled = await reconcileStaleSpotlightOnListing({
            partnerId,
            collectionName,
            listingId,
            listingData: listingDataRaw,
        });
        const listingData = reconciled.listingData || listingDataRaw;
        ...
        const pricing = resolveFeatureCheckoutAmount(
            ...
            listingData,
            {
                planId: livePlanData.planId || null,
                planIncludedSpotlight,
                collectionName,
            }
        );
        if (pricing.error) {
...

        {
            ...
            if (proratedDiffAmount <= 0) {
                ...
                if (listingId && collectionName && partnerId) {
                    ...
                    if (inlineListingRef) {
                        const inlineListingSnap = await inlineListingRef.get();
                        const inlineListingData = inlineListingSnap.exists ? inlineListingSnap.data() || {} : {};
                        ...
                        const inlineAccessThrough = updatedSubscription.current_period_end
                            ? new Date(updatedSubscription.current_period_end * 1000)
                            : null;
                        const inlineListingPatch = {
                            selectedPlan: newPlanId,
                        ...
                        const inlinePartnerRef = db.collection("partnersCollection").doc(partnerId);
                        const spotlightPatch = await applyListingSpotlightAfterPlanUpgrade({
                            listingRef: inlineListingRef,
                            listingData: inlineListingData,
                            partnerRef: inlinePartnerRef,
                            listingId,
                            collectionName,
                            newPlanId,
                            sessionId: "",
...
app.post("/api/cancel-plan", async (req, res) => {
    try {
        ...
        if (cancelFeatureOnly) {
            ...
            const listingSnap = await listingRef.get();
            if (!listingSnap.exists) {
                return res.status(404).json({ error: "Listing not found for feature cancellation." });
            }
            const listingData = listingSnap.data() || {};
            if (isSpotlightCancelPending(listingData)) {
                return res.status(409).json({
                    error: "Spotlight add-on cancellation is already scheduled for this listing.",
                });
            }
            ...
            linkedFeatureId = spotlightCtx.linkedFeatureId;
            if (!linkedFeatureId || !spotlightCtx.hasPaidStandalone) {
                return res.status(400).json({
                    error: "No active standalone spotlight add-on found for this listing.",
                });
            }
            ...
            const listingPatch = {
                featureSpotlightCancelPending: true,
                featureSpotlightCancelScope: "feature",
                featureSpotlightAccessEnd: accessEnd,
                featureSpotlightPaidThrough: accessEnd,
                updatedAt: admin.firestore.FieldValue.serverTimestamp(),
            };
            const { updated: listingsUpdated } = await applyListingPatchEverywhere(
                partnerId,
                plan.collectionName,
                plan.listingId,
                listingPatch,
        ...
        if (cancelPlan) {
            ...
            if (spotlightCascade.cancelledFeature || spotlightCascade.alreadyScheduled) {
                cancelledFeature = Boolean(spotlightCascade.cancelledFeature);
                if (spotlightCascade.linkedFeatureId) {
                    linkedFeatureId = spotlightCascade.linkedFeatureId;
                }
                await planRef.set({ spotlightEndsWithPlan: true }, { merge: true });
            }
    ...
    try {
        ...
        if (featureId) {
            ...
            if (listingSnap.exists) {
                ...
                // Billing period from Stripe subscription (month-end anchors match Stripe renewals)
                const startDate = new Date();
                const isYearly = planId?.includes("_yr");
                const billingPeriodEnd = await resolvePlanBillingPeriodEndFromCheckout(session, planId, startDate);
                const spotlightPaidThrough = billingPeriodEnd;
